...
Use: A tight binding code for individual Hamming spherical wells with fixed depth and width, as given by the well input file
...
double potAvg(int h1,int h2, int n1, int bits, int* wellWidths,double* wellDepths, int i, int j, int numWells, int* R);
double potAvgErr(int h1,int h2, int n1, int bits, int* wellWidths,double* wellDepths, int i, int j, int numWells, int* R);
...
void getInternalGap(int wellWidth, double wellDepth, int bits, int wellNum, int subspace, double s, double* internalGaps, int* wellSubspaces, double* E0, double* E1){
	...
	for(x=0; x<dim; x++){
		for(y=0; y<dim; y++){
			if(x==y){
				H[x*dim+y]=s*potOneWell(bits, wellWidth, wellDepth, x+subspace);			
			}
			else if(y==x+1){
			...
			for(x=0; x<dim; x++){
				for(y=0; y<dim; y++){
					...
					if(h1==h1p&&h2==h2p&&gamma1==gamma1p&&gamma2==gamma2p){
						H[x*dim+y]=s*potAvg(h1, h2, n1, bits, wellWidths, wellDepths, wellNum1, wellNum2, numWells, R);	
						HError[x*dim+y]=s*potAvgErr(h1, h2, n1, bits, wellWidths, wellDepths, wellNum1, wellNum2, numWells, R);	
						H1[x*dim+y]=s*pot(n1, n2, h1, h2, well1Width, 0.0, well1Depth, 0.0);
						H2[x*dim+y]=s*pot(n1, n2, h1, h2, 0.0, well2Width, 0.0, well2Depth);
					}
...
double potAvg(int h1,int h2, int n1, int bits, int* wellWidths,double* wellDepths, int i, int j, int numWells, int* R){
	...
	for(x=0; x<numWells; x++){
		if(x==i){
			...
			Vtot+=N(R[i*numWells+x], R[j*numWells+x], n1, h1, h2, wellWidths[x]-1, bits)*wellDepths[x]; 
		//	printf("%i\n", R[i*numWells+x]);			
